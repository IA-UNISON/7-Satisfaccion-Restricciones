        ...
        while pendientes:
            ...
            if temp:
                ...
                if xa not in dom_red:
                    dom_red[xa] = set({})
                dom_red[xa] = dom_red[xa].union(temp)
                pendientes += deque([(xb, xa) for xb in gr.vecinos[xa] if xa != xb ] )
...
def min_conflictos(gr, maxit=100):
    sp = [] 
    """ sp es un arreglo de listas, sp[i] contiene dos elementos s[i][0] es el valor de la variable i
        s[i][1] es el nombre de la variable i
    """
    greedy_search(gr, sp)
    for _ in range(maxit):
        a = minimos_conflictos(gr, sp)
        if a is not None:
            return a
...
def minimos_conflictos(gr, sp):
    ...
    """ 
    Se implementa el algoritmo de Minimos Conflictos, se realizan los siguiente pasos:
    
        1. Se elige al azar una variable que tenga conflicto con una o mas variables
        2. A esa variable se le asigna el valor dentro de su dominio que genera el menor numero de conflictos con otras variables
           (se rompe el empate al azar)
        3. El algoritmo para cuando ninguna variable tiene conflictos
    
    @param gr: Objeto tipo GrafoRestriccion
    @param sp: Una lista de las variables del problema con valores asignados 
               previamente por un algoritmo de busqueda greedy.
               
    
    """
    var = choice(list(x[1] for x in sp if(check_num_conflicts(x[1], x[0], sp, gr) > 0))) #Se elige un variable al azar que tenga uno o mas conflictos
    # En el siguiente ciclo for se evaluan los posibles valores de la variable X
    for valor in gr.dominio[var]:
        numConflictosCandidato = check_num_conflicts(var, valor, sp, gr)
        numConflictosActual = check_num_conflicts(var,sp[var][0], sp, gr)
        if(numConflictosCandidato < numConflictosActual):
            sp[var][0] = valor #Se elige el valor que genera menos conflictos
        if(numConflictosCandidato == numConflictosActual): #Si hay empate se elige al azar el valor
            if(randint(0,1) == 0): 
                sp[var][0] = valor
    for X in sp:
        if(check_num_conflicts(X[1], X[0], sp, gr) > 0):
            return None
    return sp
    

def check_num_conflicts(x, vx, sp, gr):
    """
    En esta funcion se cuenta el numero de conflictos para el valor vx de la variable X
    @param x: variable X para la cuál se contarán el número de conflictos.
    @param vx: valor actual de la variable X.
    @param sp: Una lista de las variables del problema con valores inicializados previamente 
               por un algoritmo de busqueda greedy.
    @param gr: Un objeto tipo GrafoRestriccion. 
    """
    numConflictos = 0
    for Y in sp:
        if x != Y[1]:
            y,vy = Y[1], Y[0]
            if(not(gr.restriccion((x,vx), (y,vy)))):
                numConflictos += 1
    
    return numConflictos

def greedy_search(gr, sp):
    """
    Este método se usa para inicializar las variables sp del problema de tal forma
    que se consiga un estado prometedor y el algoritmo de minimos conflictos
    se ejecute en menor tiempo
    
    @param sp: Una lista de las variables del problema que al momento del llamado a esta funcion cuenta con  0 elementos
    @param gr: Un objeto tipo GrafoRestriccion
    
    """
    for X in gr.dominio.keys():
        for x in gr.dominio[X]:
            numConflictos = check_num_conflicts(X, x, sp, gr)
            if(numConflictos == 0):
                sp.append(list((x,X)))
                break;
        else:
            sp.append(list((choice(list(gr.dominio.get(X))),X)))